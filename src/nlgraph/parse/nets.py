...
class Library:
    def __init__(self, name: str | LibraryType, path: Path = None):
        ...
        if path is None:
            self.path = (
                Path(__file__).parent.parent / "libraries" / self.libType.file
            ).resolve()
        else:
            self.path = path
...
class NetlistPort:
    ...
    def __eq__(self, other):
        return self.pin == other.pin and self.net == other.net

    def direction(self, regexFlavor="pin"):
        if regexFlavor == "pin":
...
class Net:
    ...
    def __eq__(self, other: Net):
        return (
            ...
            and self.netType == other.netType
        )

    @property
    def isBus(self):
        return self._busIdx is not None or (
    ...
    def __init__(
        ...
        self.netType = nType
        if "[" in n or "]" in n:
            self.name, idx = re.search(
                "(" + RegexConstants.ID_EXP + ")\[(\d+)\]", n
            ).groups()
...
class NetlistModule:
            ...
            for i, port in enumerate(entry.ports):
                ...
                if port.net.name == "1'b0" or port.net.name == "1'b1":
                    ...
                    if len(rd[port.net.name].leavingFrom) > 1:
                        raise Exception(
                            f"Defining more than 1 output for net {port.net}"
                        )
    ...
    def addEntry(self, entry: NetlistEntry):
        if entry.moduleId in self.entries:
            raise Exception("Entry already present in module")
        for port in entry.ports:
            self.addNet(port.net)
        self.entries.update({entry.moduleId: entry})

    def addEntries(self, entries: list[NetlistEntry]):
        for entry in entries:
            self.addEntry(entry)
